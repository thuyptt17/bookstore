...
<h4 style="color: #5a5c69;">BÁO CÁO TỒN KHO</h4>
<div>
    ...
    <a id="print-stock" target='_blank'>
...
<script>
    $('#view').click(function(){
        ...
        if(!isNaN(month)){ 
          if(month < current_month || (month == current_month &&current_day >=30)){        
            $.ajax({
              url:"{{url('/show-report-stock-by-month-year')}}",
              method: "POST",
              headers:{
            ...
            alert('Vui lòng nhập ngày báo cáo hợp lệ.');
          }
    });
    ...
    $('#print').click(function(){
      var date = new Date($('#input_date').val());
        ...
        if(!isNaN(month)){ 
          if(month < current_month || (month == current_month &&current_day >=30)){       
            var a = document.getElementById('print-stock');
            a.href = "{{url('/print-report-stock/')}}" + '/' + month + '/' + year;  
          } 
          else {
            alert('Tháng chưa kết thúc nên không thể lập báo cáo');
          }
